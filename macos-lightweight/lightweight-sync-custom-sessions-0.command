#!/bin/zsh
# macOS lightweight Codex setup: no Python, Node.js, Homebrew, or Desktop patch.
#
...
readonly SCRIPT_NAME="codex-lightweight-macos"
...
typeset -a ROLLBACK_KIND
typeset -a ROLLBACK_SOURCE
typeset -a ROLLBACK_BACKUP
typeset -a JSONL_PATHS
typeset -a SQLITE_PATHS
typeset -a CANDIDATE_HOME
typeset -a CANDIDATE_SOURCE
typeset -a CANDIDATE_JSONL
typeset -a CANDIDATE_THREADS
...
file_state() {
  # macOS stat format: size, mtime seconds, ctime seconds, inode.
  /usr/bin/stat -f '%z:%m:%c:%i' "$1"
}
...
atomic_restore_file() {
  ...
  local base="${source:t}"
  local temporary
  temporary="$(/usr/bin/mktemp "$directory/.${base}.rollback.XXXXXX")" || return 1
  /bin/cp -p "$backup" "$temporary" || { /bin/rm -f "$temporary"; return 1; }
  /bin/mv -f "$temporary" "$source"
}
...
rollback() {
  ...
  for (( index=${#ROLLBACK_KIND[@]}; index>=1; index-- )); do
  ...
  if [[ -n "$LEDGER_DIR" ]]; then
    if (( failed == 0 )); then
      print -r -- 'status=rolled_back' > "$LEDGER_DIR/status.txt"
    else
      print -r -- 'status=rollback_failed' > "$LEDGER_DIR/status.txt"
    fi
  fi
}
...
require_system_tools() {
  [[ -x "$PERL_BIN" ]] || die "macOS built-in Perl was not found: $PERL_BIN"
  [[ -x "$SQLITE_BIN" ]] || die "macOS sqlite3 was not found: $SQLITE_BIN. No file was changed."
  [[ -x /usr/bin/shasum ]] || die 'macOS shasum was not found. No file was changed.'
  [[ -x /usr/bin/mktemp ]] || die 'macOS mktemp was not found. No file was changed.'
  trace "Prerequisites: perl=$PERL_BIN; sqlite3=$SQLITE_BIN"
}
...
count_jsonl_files() {
  ...
  for session_scan_root in "$codex_data_dir/sessions" "$codex_data_dir/archived_sessions"; do
    ...
    while IFS= read -r -d '' ignored; do
      (( count++ ))
    done < <(/usr/bin/find "$session_scan_root" -type f -name '*.jsonl' -print0 2>/dev/null)
  done
  print -r -- "$count"
}
...
threads_count_for_database() {
  ...
  [[ -f "$database" ]] || { print -r -- '0'; return 0; }
  exists="$("$SQLITE_BIN" "$database" "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='threads';" 2>/dev/null)" || {
    print -r -- '0'
    return 0
  }
  [[ "$exists" == '1' ]] || { print -r -- '0'; return 0; }
  count="$("$SQLITE_BIN" "$database" 'SELECT count(*) FROM threads;' 2>/dev/null)" || count='0'
  [[ "$count" =~ ^[0-9]+$ ]] || count='0'
  print -r -- "$count"
}
...
add_candidate() {
  ...
  if [[ -d "$raw" ]]; then
    candidate_dir="$(cd "$raw" && /bin/pwd -P)" || return 0
...
add_managed_patch_candidates() {
  ...
  for root in "$HOME/Applications" '/Applications'; do
    ...
    # BSD find on macOS has no GNU -maxdepth flag. These two roots are small
    # application locations, and the filename is specific to this patcher.
    done < <(/usr/bin/find "$root" -type f -name '.codex-gpt56-patch.json' -print0 2>/dev/null)
  done
}
...
select_codex_home() {
  ...
  local index nonempty=0 selected=0
  for (( index=1; index<=${#CANDIDATE_HOME[@]}; index++ )); do
  ...
  if (( nonempty == 0 )); then
    ...
    if [[ -n "${1:-}" ]]; then
      SELECTED_HOME="${CANDIDATE_HOME[1]}"
    else
      for (( index=1; index<=${#CANDIDATE_HOME[@]}; index++ )); do
      ...
      [[ -n "$SELECTED_HOME" ]] || SELECTED_HOME="${CANDIDATE_HOME[1]}"
    fi
  ...
  for (( index=1; index<=${#CANDIDATE_HOME[@]}; index++ )); do
    printf '  [%d] JSONL %s; SQLite threads %s\n      %s\n      Source: %s\n' \
      "$index" "${CANDIDATE_JSONL[$index]}" "${CANDIDATE_THREADS[$index]}" \
  ...
  while true; do
    ...
    SELECTED_HOME="${CANDIDATE_HOME[$reply]}"
    return 0
  done
}

matching_processes() {
  /usr/bin/pgrep -x 'Codex|ChatGPT' 2>/dev/null || true
}
...
prepare_ledger() {
  ...
  identity="$(print -rn -- "$SELECTED_HOME" | /usr/bin/shasum -a 256 | /usr/bin/awk '{print substr($1,1,12)}')"
  stamp="$(/bin/date '+%Y%m%d-%H%M%S')-$identity-$RANDOM"
  LEDGER_DIR="$SELECTED_HOME/backups/codex-gpt56/lightweight-native-macos/$stamp"
  /bin/mkdir -p "$LEDGER_DIR/data/config" "$LEDGER_DIR/data/jsonl" "$LEDGER_DIR/data/sqlite" || die 'Could not create the migration ledger.'
  /bin/chmod 700 "$LEDGER_DIR" "$LEDGER_DIR/data" || true
  {
    print -r -- 'format=codex-lightweight-macos-v1'
    print -r -- "codex_home=$SELECTED_HOME"
    print -r -- "target_provider=$TARGET_PROVIDER"
    print -r -- "created_at=$(/bin/date -u '+%Y-%m-%dT%H:%M:%SZ')"
    print -r -- 'status=changing'
  } > "$LEDGER_DIR/manifest.txt"
  : > "$LEDGER_DIR/entries.tsv"
  print -r -- 'status=changing' > "$LEDGER_DIR/status.txt"
  trace "LEDGER created: $LEDGER_DIR"
}

make_sibling_temp() {
  local source="$1"
  /usr/bin/mktemp "${source:h}/.${source:t}.codex-lightweight.XXXXXX"
}
...
rewrite_live_config() {
  ...
  /bin/mkdir -p "$SELECTED_HOME" || die "Could not create CODEX_HOME: $SELECTED_HOME"
  ...
  if [[ -f "$config" ]]; then
    ...
    /bin/cp -p "$config" "$backup" || { /bin/rm -f "$temporary"; die 'Could not back up config.toml.'; }
    [[ "$(sha256_file "$backup")" == "$before_hash" ]] || { /bin/rm -f "$temporary"; die 'Config backup verification failed.'; }
    ...
    /bin/chmod "$mode" "$temporary" || true
    /bin/mv -f "$temporary" "$config" || die 'Atomic config replacement failed.'
    remember_rollback 'file' "$config" "$backup"
  else
    append_manifest 'config' "$config" '' 'created'
    /bin/mv -f "$temporary" "$config" || die 'Could not create config.toml.'
...
collect_jsonl_paths() {
  ...
  for scan_root in "$SELECTED_HOME/sessions" "$SELECTED_HOME/archived_sessions"; do
    ...
    done < <(/usr/bin/find "$scan_root" -type f -name '*.jsonl' -print0 2>/dev/null)
  done
...
collect_sqlite_paths() {
  ...
  if [[ -n "$external" && -f "$external/state_5.sqlite" ]]; then
    ...
    for prior in "${SQLITE_PATHS[@]:-}"; do [[ "$prior" == "$candidate" ]] && candidate=''; done
...
migrate_jsonl_file() {
  ...
  path_hash="$(print -rn -- "$source" | /usr/bin/shasum -a 256 | /usr/bin/awk '{print substr($1,1,16)}')"
  backup="$LEDGER_DIR/data/jsonl/$path_hash/${source:t}"
  /bin/mkdir -p "${backup:h}" || { /bin/rm -f "$temporary"; die 'Could not create JSONL backup directory.'; }
  /bin/cp -p "$source" "$backup" || { /bin/rm -f "$temporary"; die "Could not back up JSONL: $source"; }
  [[ "$(sha256_file "$backup")" == "$before_hash" ]] || { /bin/rm -f "$temporary"; die "JSONL backup verification failed: $source"; }
  ...
  /bin/chmod "$mode" "$temporary" || true
  /bin/mv -f "$temporary" "$source" || die "Atomic JSONL replacement failed: $source"
  ...
  if ! rewrite_jsonl_to_temp "$source" "$verify_temp" || ! /usr/bin/cmp -s "$source" "$verify_temp"; then
    /bin/rm -f "$verify_temp"
  ...
  local count
  count="$(print -r -- "$output" | /usr/bin/sed -n 's/^changed_lines=\([0-9][0-9]*\).*/\1/p')"
...
migrate_sqlite_database() {
  ...
  path_hash="$(print -rn -- "$database" | /usr/bin/shasum -a 256 | /usr/bin/awk '{print substr($1,1,16)}')"
  backup="$LEDGER_DIR/data/sqlite/$path_hash/${database:t}"
  /bin/mkdir -p "${backup:h}" || die 'Could not create SQLite backup directory.'
...
SQL
  ...
  changed="$(print -r -- "$changed" | /usr/bin/tail -n 1 | /usr/bin/tr -d '[:space:]')"
...
main() {
  ...
  printf '\nCodex lightweight session unifier (macOS built-in tools)\n\n'
  ...
  printf 'Not modified      : CC Switch database, model catalogs, Desktop app bundle, session bodies\n\n'
  ...
  if /usr/bin/pgrep -ifl 'cc[ -]?switch' >/dev/null 2>&1; then
  ...
  if ! /usr/bin/cmp -s "$SELECTED_HOME/config.toml" "$check_temp"; then
    /bin/rm -f "$check_temp"
  ...
  print -r -- 'status=complete' > "$LEDGER_DIR/status.txt"
  print -r -- 'status=complete' >> "$LEDGER_DIR/manifest.txt"
  ROLLBACK_ENABLED=0
  RUN_SUCCEEDED=1
  local elapsed=$(( $(/bin/date +%s) - START_EPOCH ))
  printf '\n'
  ...
  warn 'This lightweight script does not patch the Desktop model-picker UI; it only sets the root default reasoning effort to xhigh.'
}
